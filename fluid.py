...
u0 = .03  # initial and in-flow speed
...
def next_frame(arg):  #arg is the frame number
    ...
    #bImageArray[n, 2] = int(n)
    #bImageArray[:, 0] = np.asarray(fluid_image_density)
...
def construct_barrier(height, width):
    barrier_object.box((height / 2 - 2 * height / 5), (height - height / 2 + 2 * height / 5), (width / 2 - width / 5),
                       (width / 2 + width / 5))
    barrier_object.semicircle(2 * height / 5, -90, 90, width / 2 + width / 5, height / 2 + 1)
    barrier_object.vertical_line(width / 2 + width / 5, height / 2 - 2 * height / 5 + 1,
                                 height / 2 + 2 * height / 5 - 1)
    barrier_object.vertical_line(width / 2 - width / 5, height / 2 - 2 * height / 5 + 1,
                                 height / 2 + 2 * height / 5 - 1)
    barrier_object.point(74, 90)
    barrier_object.point(74, 10)
    barrier_object.semicircle(1.99 * height / 5, 90, 270, width / 2 - width / 5, height / 2 + 1)
    # barrier_object.box(0, height-1, 0, width-1)

    #destroyer_object = Destroyer(height, width, 1)
    #ang = destroyer_object.simpleAngle()
    #loc = [height / 2, width / 2]
    #reached_barrier = False
    #while not reached_barrier:
    #    loc += [np.cos(ang*np.pi/180), np.sin(ang*np.pi/180)]
    #    if barrier_object.barrier[int(loc[0]), int(loc[1])] == True:
...
def construct_density(height, width):
    density_object.boxDensity(4 * height / 5, 2 * width / 5, height / 10, width / 2 - width / 5, 5)
    density_object.boxDensity(height, width, 0, 0, 1)
    density_object.semicircleDensity(2*height/5, -90, 90, width/2+width/5, height/2+1, 5)
    density_object.semicircleDensity(2*height/5, 90, 270, width / 2 - width / 5, height / 2 + 1, 5)

def destroy_barrier():
    loc = [height / 2 + 2 * height / 5, width / 2]
    barrier_object.barrier[int(loc[0]), int(loc[1] - 5):int(loc[1] + 5)] = False
    bImageArray[barrier, 3] = 255
    plt.imshow(bImageArray, origin='lower', interpolation='none')

# get barrier, density, destroyer ---------------------------------
...
bImageArray[barrier, 3] = 64  # set alpha=255 only at barrier sites
